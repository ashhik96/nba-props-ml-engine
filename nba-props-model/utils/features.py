...
def calculate_double_double_probability(game_logs_df):
    """
    OLD logic you originally had:
    Probability (as fraction 0-1) of recording a double-double
    using only this season (player_game_logs).
    We are not using this directly anymore, but leaving it here
    in case other code imports it.
    """
    if game_logs_df is None or game_logs_df.empty:
        return 0.0
    
    def is_double_double(row):
        # 10+ in at least 2 of PTS / REB / AST
        double_stats = sum([
            row['PTS'] >= 10,
            row['REB'] >= 10,
            row['AST'] >= 10
...
def _double_double_rate(df):
    """
    NEW helper:
    Return fraction of games (0.0-1.0) where the player recorded a double-double:
    10+ in at least TWO of {PTS, REB, AST}.
    If df is None/empty, or missing cols, return None (so we know it's unknown).
    """
    if df is None or df.empty:
        return None

    needed_cols = {'PTS', 'REB', 'AST'}
    if not needed_cols.issubset(df.columns):
        return None

    # boolean masks: did they hit 10+ in each category?
    pts10 = df['PTS'] >= 10
    reb10 = df['REB'] >= 10
    ast10 = df['AST'] >= 10

    # 2+ categories hit 10+ → double-double
    dd_mask = (pts10.astype(int) + reb10.astype(int) + ast10.astype(int)) >= 2
    if len(dd_mask) == 0:
        return None

    return float(dd_mask.mean())
...
def analyze_head_to_head_performance(h2h_games_df, stat_type):
    ...
    """
    if h2h_games_df is None or h2h_games_df.empty:
        return {
            'h2h_avg': 0.0,
            'h2h_games': 0,
            'h2h_trend': 0.0
...
def is_back_to_back(game_logs_df, game_index=0):
    ...
    """
    if game_logs_df is None or game_logs_df.empty or len(game_logs_df) < 2:
        return False
...
def build_enhanced_feature_vector(
    ...
    - Recency stats (last5 / last10, stabilized by mixing prior season if needed)
    - Opponent context
    - Head-to-head vs this exact team
    - Game context (rest, back-to-back)
    - Double-double probability (blended new logic)
    """
    ...
    if (player_game_logs is not None and not player_game_logs.empty) and \
       (prior_season_logs is not None and not prior_season_logs.empty):
        ...
        # no data at all, fall back to blended averages
        last5_stats = {}
        last10_stats = {}
    ...
    # Double-double probability (BLENDED NEW LOGIC)
    # -------------------------
    # We combine this season and last season for stability so:
    # - A guy isn't stuck at 0% just because he hasn't had one YET
    # - A guy with 1 DD in 1 game doesn't get 100%
    curr_dd = _double_double_rate(player_game_logs)       # 0.0-1.0 or None
    prev_dd = _double_double_rate(prior_season_logs)      # 0.0-1.0 or None
    ...
    cur_n = _count_games(player_game_logs)
    prev_n = _count_games(prior_season_logs)

    if curr_dd is not None and cur_n >= 10:
        blended_dd = curr_dd
    elif curr_dd is not None and prev_dd is not None:
        blended_dd = 0.6 * curr_dd + 0.4 * prev_dd
    elif curr_dd is not None:
        blended_dd = 0.6 * curr_dd
    elif prev_dd is not None:
        blended_dd = 0.9 * prev_dd
    else:
        blended_dd = 0.0

    # Store as probability (0.0 - 1.0). The app multiplies by 100 later.
    features['dd_probability'] = blended_dd
